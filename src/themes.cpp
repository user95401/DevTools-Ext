...
static std::map<std::string, std::function<void()>> THEMES = {
	{ "1. Unreal (by dev0-1)", []() {
...
size_t getThemeIndex(std::string const& theme) {
    return ranges::indexOf(getThemeOptions(), theme).value_or(0);
}

std::string getThemeAtIndex(size_t index) {
    return getThemeOptions().at(index);
}
